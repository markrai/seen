...
REM Docker/WSL web deployments use the backend-powered /browse file browser.
REM Enable it at build-time for the web bundle.
set "VITE_ENABLE_FILE_BROWSER=1"
echo File browser enabled (Docker/WSL web): %VITE_ENABLE_FILE_BROWSER%
call npm run build
if errorlevel 1 (
...
if "%1"=="--facial-recognition" (
    ...
    echo Usage: run_with_docker.bat [--facial-recognition]
    echo.
    echo Options:
